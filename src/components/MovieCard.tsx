...
const MovieCard = ({ movie }: Props) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="flex flex-col items-start gap-2 absolute left-0 right-0 top-full
                bg-black p-3 rounded-b-md
                transition-all duration-200 text-white"
      >
        <p className="font-semibold text-lg ">{movie.title}</p>
        <p className="text-xs">
          Release Date: {movie.release_date ? formatDate(movie.release_date) : 'TBA'}
        </p>
        <p className="text-xs">
          Rating: {movie.vote_count > 0 ? movie.vote_average.toFixed(1) : 'TBA'}
        </p>
        <p
          className="text-xs group-hover:mt-4 max-h-0 overflow-hidden opacity-0
                transition-all duration-200
                group-hover:max-h-40 group-hover:opacity-100 text-left group-focus-within:max-h-40 group-focus-within:opacity-100"
        >
